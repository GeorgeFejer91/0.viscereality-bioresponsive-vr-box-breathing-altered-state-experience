...
7. Do not use em dashes in project text. Use commas, parentheses, or separate sentences as appropriate. Use a colon only when it adds necessary clarity and the colon-frequency rule below permits it.
...
14. Use colons sparingly. Normally use no more than one colon, and never more than two, within the Introduction or any other comparably bounded manuscript section. Do not use colons as a routine device for emphasis, explanation, or list introduction.
15. Limit every prose enumeration to no more than three coordinate items, including lists of examples, phenomena, or main characteristics. When scientific completeness requires more material, group it under accurate higher-order terms or distribute it across separate sentences, each with no more than three parallel items. Do not omit essential information merely to meet this rule.
16. In an enumeration, place each citation immediately after the specific item or claim it supports whenever the sources can be mapped separately. Put citations at the end of a list only when the same source or source set supports the complete list.
17. Cite every externally verifiable concrete claim for which an appropriate source exists, including empirical and methodological claims. Common knowledge, logical transitions, and explicit authorial interpretations do not require decorative citations. The current paper's own directly reported methods, preregistered hypotheses, and results are primary reporting, but cite or internally cross-reference their source when the basis is not adjacent. Never attach an irrelevant citation merely to create citation density.
